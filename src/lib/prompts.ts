...
ニュース記事の内容を、公開データソース（GDELT メディアトーン分析）の定量データと照合し、
記事の主張がデータで裏付けられるか、矛盾するか、検証不能かを判定してください。
...
[
  {
    ...
    "evidence": "データに基づく検証結果の説明（150字以内）",
...
- contradicted: 記事の主張がデータと矛盾している（例: 「緊張緩和」と書いているがトーンは悪化）
- unverifiable: 記事のテーマに対応するOSINTデータがない、または判定困難
- 必ず具体的な数値を引用すること（例: メディアトーンが-2.1→-3.5に悪化）`;

export const ANALYST5_NOVEL_ARTICLE_PROMPT = `あなたはOSINTインテリジェンスアナリストです。
公開データソース（GDELTメディアトーン分析）で検出された異常値から、
まだニュースとして報じられていない可能性のある事象を類推し、独自分析記事を生成してください。
出力はJSON形式のみで返してください。前文やMarkdownのバックティックは不要です。

【出力JSON形式】
{
  "title": "記事タイトル（日本語、30字以内）",
  "body": "本文（日本語、800字程度。データに基づく分析、考えられる要因、今後の展開予測を含める）",
  "theme": "最も関連するテーマID",
...
注意:
- 「まだ報じられていない」事象の類推であることを明示すること
- データの異常値から論理的に推論可能な範囲に留めること
